<!DOCTYPE HTML>
<html>

<head>
<style>
    body {
        background: linear-gradient(to left, blue, skyblue,blue);
        color: black;/* Warna teks */
        font-family: 'Arial', sans-serif; /* Gaya font */
        margin: 0;
        padding: 0;
        text-align: center;
    }

    #chartContainer {
        height: 250px;
        max-width: 520px;
        margin: 20px auto; /* Jarak dari atas dan bawah */
        border: 5px solid purple; /* Warna garis pinggir grafik */
        border-radius: 5px; /* Border radius untuk sudut grafik */
    }

    h2 {
        color: black; /* Warna judul */
    }

    #indikator {
        color: black; /* Warna teks indikator */
        font-size: 20px;
        margin-top: 20px;
    }
</style>

</head>

<body>
    <h1>CARIO MADA ARDISTO</h1>
    <h1>G.211.21.0040</h1>
    <!-- menampilkan grafik dengan id chartContainer -->
    <!-- ukuran grafik: tinggi = 550 piksel, dan maksimal lebar 920 piksel -->
    <div id="chartContainer" style="height: 250px; max-width: 520px; margin: 0px auto;"></div>

    <h2 style="text-align: center">Monitoring Suhu Real Time</h2>

    <!-- import libaray canvasjs dan jquery dengan cdn  -->
    <script src="https://canvasjs.com/assets/script/canvasjs.min.js"></script>
    <script src="https://code.jquery.com/jquery-3.5.1.js"
        integrity="sha256-QWo7LDvxbWT2tbbQ97B53yJnYU3WhH/C8ycbRAkjPDc=" crossorigin="anonymous"></script>

    <script>

        function beep() {
            var snd = new Audio("data:audio/wav;base64,");
            snd.play();

        }


        window.onload = function () {
            //inisialisasi data array dps kepanjangan dari datapoints
            var dps = [];
            var dataLength = 10; // panjang data yang ditampilkan (horizontal), ditampilkan di bagian bawah grafik
            var updateInterval = 1000; //setiap 1,5 dtk data direfresh
            var xVal = 0;
            var yVal = 0;

            //inisialisasi chart js
            var chart = new CanvasJS.Chart("chartContainer", {
                title: {
                    text: "Grafik Suhu Realtime" //memberi judul grafik
                },
                data: [{
                    type: "area", //tipe grafik yang digunakan, lihat di situsnya untuk lihat gaya lain
                    dataPoints: dps //dps adalah data yang digunakan
                }]
            });

            var updateChart = function (count) {
                // data json diambil pada alamat /getdata
                $.getJSON("http://localhost/proyek-akhir/html_php/iot/monitor/getdata.php", function (data) {
                    // $.getJSON("https://api.coindesk.com/v1/bpi/currentprice.json", function (data) {
                    // var suhu = data.bpi.USD.rate_float //mengambil data spesifik rate_float
                    var suhu = data.suhu //mengambil data spesifik rate_float
                    if (suhu < 29) {
                        document.getElementById("indikator").textContent = "Suhu : " + suhu.toString() + " NORMAL : Aman ";
                        // document.getElementById("huruf").color="GREEN"; 

                    }

                    if (suhu >= 29 && suhu < 30) {
                        document.getElementById("indikator").textContent = "Suhu :" + suhu.toString() + " NORMAL : * ";
                        // document.getElementById("huruf").color="BLUE"; 

                    }

                    if (suhu >= 30 && suhu < 31) {
                        document.getElementById("indikator").textContent = "Suhu : " + suhu.toString() + " PANAS : ** "; beep();
                    }
                    if (suhu >= 31) {
                        document.getElementById("indikator").textContent = "Suhu : " + suhu.toString() + " SANGAT PANAS : *** "; beep(); beep(); beep();
                    }





                    console.log(suhu) //menampilkan data dengan console.log hanya terlihat saat mode inspect element
                    yVal = suhu // mengisi variabel yVal dengan data usd
                    count = count || 1;

                    //melakukan perulangan data dengan for agar data dapat dijalankan
                    for (var j = 0; j < count; j++) {
                        dps.push({
                            x: xVal,
                            y: yVal
                        });
                        xVal++;
                    }

                    //jika datapoints telah melewati datalength
                    if (dps.length > dataLength) {
                        dps.shift(); //maka hapus data awal dengan fungsi shift()
                    }

                })
                chart.render();
            };

            //jalankan fungsi updateChart di atas
            updateChart(dataLength);

            //fungsi agar data dapat diupdate setiap 1000 detik sekali
            setInterval(function () {
                updateChart()
            }, updateInterval);
        }

    </script>
    <center>
        <div id=indikator> <b> NORMAL </b> </div>
    </center>
</body>

</html>